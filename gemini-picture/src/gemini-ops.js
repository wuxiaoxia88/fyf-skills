...
export function createOps(page) {
  ...
  return {
   ...
   async extractImageBase64(url) {
     ...
     * 完整生图流程：发送提示词 → 等待 → 提取图片
     *
     * 关键修复：sendAndWait 只等文字回复（'mic'状态），图片可能在文字回复之后
     * 才完全渲染。因此在 sendAndWait 之后，需要主动等"新图片出现"，确保拿到
     * 的是本轮生成的图，而非上一轮的旧图。
     *
     * @param {string} prompt
     * @param {object} [opts]
     * @param {number} [opts.timeout=120000]
     * @param {boolean} [opts.fullSize=false]
     * @param {(status: object) => void} [opts.onPoll]
     */
    async generateImage(prompt, opts = {}) {
      const { timeout = 120_000, fullSize = false, onPoll } = opts;

      // ── 记录当前图片数量（用于判断是否有新图出现） ──
      const imgCountBefore = await op.query(() =>
      ...
      if (!waitResult.ok) {
        const limit = await op.query(() => {
          const body = document.body.innerText;
          if (/已达到.*(?:图片|生成).*限额|you've?\s*reached\s*(?:your\s*)?(?:image\s*)?(?:generation\s*)?limit|429\s/i.test(body)) {
            const patterns = [
              /已达到[^\n]{0,100}/,
              /you've?\s*reached[^\n]{0,100}/i,
              /429[^\n]{0,20}/,
            ...
            for (const p of patterns) {
              ...
              if (m) return { limited: true, message: m[0].slice(0, 200) };
            }
            return { limited: true, message: 'rate_limit_detected' };
          }
          return { limited: false };
        });
        if (limit.limited) {
          console.warn(`[generateImage] ⚠️ Rate Limit: ${limit.message}`);
          return { ok: false, error: 'rate_limit', rateLimitMessage: limit.message, elapsed: waitResult.elapsed };
        }
        return { ...waitResult, step: 'sendAndWait' };
      }

      // ── 等待新图片出现（关键修复：不等固定时间，等图片数量增加） ──
      const imgAppearTimeout = 60_000; // 最多等 60 秒让图片渲染
      const imgAppearStart = Date.now();
      let imgAppeared = false;

      while (Date.now() - imgAppearStart < imgAppearTimeout) {
        const imgCountAfter = await op.query(() =>
          document.querySelectorAll('img.image.loaded').length
        );
        if (imgCountAfter > imgCountBefore) {
          imgAppeared = true;
          console.warn(`[generateImage] ✅ 新图出现（${imgCountBefore} → ${imgCountAfter}），等待渲染稳定...`);
          await sleep(2000); // 多等 2 秒让渲染完全
          break;
        }
        await sleep(500);
      }
      ...
      if (!imgAppeared) {
        console.warn(`[generateImage] ⚠️ 等待 ${imgAppearTimeout / 1000}s 未检测到新图片，继续尝试提取`);
      }
