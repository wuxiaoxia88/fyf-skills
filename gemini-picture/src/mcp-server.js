...
- 必须等到拿到最终成功/失败结果后，再向用户回传产物（文件路径）或报告错误。`,
    inputSchema: {
      ...
      downloadTimeout: z.number().default(300000).describe(
        "仅在 fullSize=true 时生效，等待原图下载完成的超时时间（毫秒），默认 300000（5 分钟）"
      ),
  ...
  async ({ prompt, newSession, referenceImages, fullSize, downloadTimeout, timeout }) => {
    try {
      ...
      const result = await ops.generateImage(prompt, { fullSize, downloadTimeout, timeout });
